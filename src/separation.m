    ...
    while i < lrn
        ...
        if (c > 3) && (rn(i, lrn2) == 0)
            ...
            for j = 1:numsplitmodes
                ...
                %Clist is a 2*connectivitymax array, the 2 lines refers to the node which has been split and the new one wich has been created. The first column is the connectivity
                clist = zeros(2, cmax);
                ...
                [vntmp, ~] = mobility(fseg, rn, links, connectivity, nodelist, clist, Bcoeff, rotMatrix);
                vd1 = vntmp(1, :) * vntmp(1, :)';
                vd2 = vntmp(2, :) * vntmp(2, :)';
                ...
                [vntmp, fntmp] = mobility(fseg, rn, links, connectivity, nodelist, clist, Bcoeff, rotMatrix);
