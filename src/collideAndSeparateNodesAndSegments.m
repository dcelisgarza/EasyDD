    ...
    if (docollision)
        ...
        while colliding_segments == 1
            ...
            if colliding_segments == 1%scan and update dislocation structure.
                ...
                [rnnewTmp, linksnewTmp, ~, ~, fsegnewTmp, colliding_segments, collided] = collision(rnnew, linksnew, connectivitynew, ...
                    linksinconnectnew, fsegnew, rann, MU, NU, a, Ec, mobility, vertices, rotMatrix, u_hat, nc, xnodes, ...
                    D, mx, my, mz, w, h, d, floop, n1s1, n2s1, n1s2, n2s2, s1, s2, segpair, lmin, CUDA_flag, Bcoeff);
                ...
                if collided
                    ...
                    [rnnewTmp, linksnewTmp, connectivitynewTmp, linksinconnectnewTmp, fsegnewTmp, separated] = separation(doseparation, rnnewTmp, ...
                        linksnewTmp, connectivitynewTmp, linksinconnectnewTmp, fsegnewTmp, mobility, rotMatrix, MU, NU, a, Ec, ...
                        2 * rann, vertices, u_hat, nc, xnodes, D, mx, my, mz, w, h, d, CUDA_flag, Bcoeff);
                    numNodeTmp = size(rnnewTmp, 1);
                    numSegTmp = size(linksnewTmp, 1);

                    if separated && numNodeTmp == numNode && numSegTmp == numSeg% || numNodeTmp == numNode - 1 && numSegTmp == numSeg - 1
                        s1Skip = [s1Skip; s1];
                        s2Skip = [s2Skip; s2];
        ...
        % Sometimes the above causes us to skip some collision-separations
        % that should be performed, here we just collide them and the
        % outside separation takes care of them.
        colliding_segments = 1;
        s1Skip = [];
        s2Skip = [];
        while colliding_segments == 1
            ...
            if colliding_segments == 1
                [rnnew, linksnew, ~, ~, fsegnew, colliding_segments, collided] = collision(rnnew, linksnew, connectivitynew, ...
                    linksinconnectnew, fsegnew, rann, MU, NU, a, Ec, mobility, vertices, rotMatrix, u_hat, nc, xnodes, ...
                    D, mx, my, mz, w, h, d, floop, n1s1, n2s1, n1s2, n2s2, s1, s2, segpair, lmin, CUDA_flag, Bcoeff);
